<script setup>
import { SOCIAL_GRAPHIC_GENERATOR_QUERY } from '@/queries/socialGraphicGenerator.mjs'
...
const { data } = await useAsyncData(
  ...
  async () => {
    ...
    if (!result?.entry) {
      throw createError({
        statusCode: 404,
        message: 'Social graphic generator page not found',
      })
    }
...
useHead(() => ({
  title: page.value.metaTitle || page.value.title || 'New Starter',
  meta: [
    {
      name: 'description',
...
async function downloadGraphic() {
  ...
  try {
    ...
    output.toBlob((blob) => {
      ...
      link.href = url
      link.download = `${uploadedFileName.value}-social-graphic.jpg`
  ...
  <div class="sgn-page-wrapper">
    <div class="container">
      <header class="sgn-page-header">
        <h1 class="sgn-page-title">New Starter Social Graphic Generator</h1>
      ...
      <section id="sgn-section-upload" class="sgn-card">
        ...
        <div class="sgn-upload-area">
          <label class="sgn-file-input-wrapper" for="sgn-image-upload">
