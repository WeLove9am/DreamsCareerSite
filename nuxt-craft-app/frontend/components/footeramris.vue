                ...
                <div class="footer__middle">
                    ...
                    <ul class="footer__nav">
                        <li class="footer__col" v-for="item in footerData" :key="item.id">
                            <a
                                class="footer__title"
                                :href="item.pageLink.startsWith('http')
                                    ? item.pageLink
                                    : `https://careers.dreams.co.uk/${item.pageLink.startsWith('/') ? item.pageLink.slice(1) : item.pageLink}`"
                            >
                                <h4>{{ item.title }}</h4>
                            </a>
                            <ul class="footer__list">
                                <li v-for="child in item.children" :key="child.id">
                                    <a 
                                    :href="child.pageLink.startsWith('http')
                                            ? child.pageLink
                                            : `https://careers.dreams.co.uk/${child.pageLink.startsWith('/') ? child.pageLink.slice(1) : child.pageLink}`"
